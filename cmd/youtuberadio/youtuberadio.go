...
import (
	...
	rtmp "github.com/HugoGuiroux/gortmp"
...
func main() {
	...
	// RTMP server
	l := zlog.NewLogger(".", "publisher", nil, 60, 3600, true)
	l.SetMainLevel(zlog.LOG_LEVEL_DEBUG)
	rtmp.InitLogger(l)
	defer l.Close()
	...
	fmt.Println("to dial")
	fmt.Println("a")
	obConn, err = rtmp.Dial(info.URL, testHandler, 100)
	if err != nil {
	...
	for {
		...
		case stream := <-createStreamChan:
			...
			err = stream.Publish(info.Name, "live")
			if err != nil {
